using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class PuzzleConcertarValvula : MonoBehaviour
{
    EstadoDeMundo estado;
    DialogueTrigger DTPlayer;

    private void Start()
    {
        estado = FindObjectOfType<EstadoDeMundo>();

        if (estado.save.turno != 9 ||
            estado.save.fimIntroducaoTurno9 == false
            || estado.save.puzzleConcertouMaquinas9 == true)
        {
            Destroy(this.gameObject);
        }

        DTPlayer = FindObjectOfType<DialogueTriggerPlayer>();
    }
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (estado.save.puzzleConcertouMaquinas9 == true)
            Destroy(this.gameObject);

        else if (estado.save.puzzleConcertouMaquinas9 == false)
        {
            estado.save.puzzleConcertouMaquinas9 = true;
            DTPlayer.StartDialogue(201, 202);
            Destroy(this.gameObject);
        }


    }
}
